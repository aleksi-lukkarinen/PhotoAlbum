...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Renaming column for 'Order.status' to match new field type.
        db.rename_column('albumizer_order', 'status', 'status_id')
        # Changing field 'Order.status'
        db.alter_column('albumizer_order', 'status_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['albumizer.OrderStatus']))

        # Adding index on 'Order', fields ['status']
        db.create_index('albumizer_order', ['status_id'])


    def backwards(self, orm):
        
        # Removing index on 'Order', fields ['status']
        db.delete_index('albumizer_order', ['status_id'])

        # Renaming column for 'Order.status' to match new field type.
        db.rename_column('albumizer_order', 'status_id', 'status')
        # Changing field 'Order.status'
        db.alter_column('albumizer_order', 'status', self.gf('django.db.models.fields.IntegerField')())
    ...
    models = {
        ...
        'albumizer.shoppingcartitem': {
            'Meta': {'ordering': "['user', 'album']", 'unique_together': "(('user', 'album'),)", 'object_name': 'ShoppingCartItem'},
